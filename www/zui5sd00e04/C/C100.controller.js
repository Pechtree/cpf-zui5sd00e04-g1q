sap.ui.controller("zui5sd00e04.C.C100", {
	//--------------------------------------------------------------------------------	
	//PAI FCODE 
	//--------------------------------------------------------------------------------	
	M01_FCD: function(fcode,oModela){
		...
		if(fcode=='c101btnDis'){//Display Raw Meat
			...
			oApp.diaMeatEdit_PBO('c101','c101TblPmx','menge_ist');
		};
		...
		if(fcode=='c101btnCalOk'){//Adjust Raw Meat - OK
			...
			oCon.getCell('c101TblPmx',6).setText(oCon.fmtDec(oCon.getControl('c101inpMenge_ist').getValue(),3));
			oCon.getControl('c101DiaCal').close();
		};
		
		if(fcode=='c101btnAcp'){//Edit - OK
			oCon.getCell('c101TblPmx',6).setText(oCon.fmtDec(oCon.getControl('c101inpOrder').getValue(),3));
		};
		
		if(fcode=='c101LstMeat'){ //RAW Meat Selected
			if(!oCon.isSelected('c101LstMeat')){return;};
			oApp.diaMeatEdit_PBO('c101','c101LstMeat','menge_ist');
	...
	M02_PAR: function(fcode,oModela){
		...
		if(fcode=="SAPEVT_C102"){
			return oParameters = { 
				"idate" 	: oCon.getControl('c103inpDate').getValue(),
				"iapriod" 	: oCon.getAbapTrue('c103chkShiftD'),
				"iaprion" 	: oCon.getAbapTrue('c103chkShiftN'),
				"aufnr" 	: oCon.arrListItemsNoBlank('c101TblPmx','aufnr','menge_ist'),
				"matnr_px" 	: oCon.arrListItemsNoBlank('c101TblPmx','matnr_px','menge_ist'),
				"menge_ist" : oCon.arrListItemsNoBlank('c101TblPmx','menge_ist','menge_ist'),};
		};
		...
		if(fcode=="SAPEVT_C106"){//Get RAW Meat
			return oParameters = { 
				...
				"imenge"   : oCon.getProperty('c101TblPmx','menge_ist'),
			};
		};
